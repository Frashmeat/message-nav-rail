...
message-nav-rail 是 Oh My Pi 的会话导航扩展。它会在输入框上方用白点显示用户输入，模型回复不会生成节点；新用户消息生成小点后会自动选中最新项，选中白点后可以预览内容并跳转到对应消息。
