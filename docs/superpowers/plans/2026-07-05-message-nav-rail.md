...
**当前状态（2026-07-21 核对）：** 已用本地 Oh My Pi `17.0.6` 维护源码核对扩展 API。当前项目以 `types/pi-coding-agent.d.ts` 保存实际使用的宿主类型子集，并在运行时兼容两类已出现事件 payload：`event.message.role/content` 与 `event.role/text/content/messageId`。导航栏只保留用户输入，assistant entry 与流式输出不生成节点。
...
- `setLabel`、`pi.on(event, handler(event, ctx))`、`registerShortcut(shortcut, options)`：已结合 `17.0.6` 维护源码确认。
- `ctx.ui.setWidget(key, content, options)`、`ctx.ui.notify`：已结合 `17.0.6` 维护源码确认。
