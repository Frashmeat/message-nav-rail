...
当前构建基线为 Oh My Pi `17.0.6`：锁定 `can1357/oh-my-pi` 稳定标签对应提交 `89d6a8f6d14286f32f09ec9c8aa8af7b3451d2d6`，再应用 `patches/oh-my-pi/17.0.6-release-windows-x64.patch`。这使 GitHub Actions 不依赖尚未推送的本地 fork 合并提交。
...
- 构建 Action [`29836615333`](https://github.com/Frashmeat/message-nav-rail/actions/runs/29836615333) 已成功，源码提交为 `bd6235c540e1c7f91d89e88ce5372c5daf5b3439`。
- [`v17.0.6-custom.2`](https://github.com/Frashmeat/message-nav-rail/releases/tag/v17.0.6-custom.2) 已发布为 prerelease；`custom.1` 因普通 Windows 用户缺少符号链接权限时安装失败而被取代。
- Release 资产为 `message-nav-rail-omp-17.0.6-custom.2-windows-x64.zip` 及对应 `.zip.sha256`。
- 发布 ZIP 的 SHA-256 为 `739578cb08a65393f1c61eb590b161f29a88111d6b85d6d24c1dbb932ec5eaba`。
- 当前 prerelease 已完成构建、打包、内嵌 native 探测、静态完整性验收及 Windows PowerShell 5.1 隔离完整安装；独立 Windows 10/11 目标机的升级与故障回滚验收仍待执行。

## 上游升级状态（2026-07-21）

- 本地维护分支已合并官方稳定版 `v17.0.6`，上游提交为 `89d6a8f6d14286f32f09ec9c8aa8af7b3451d2d6`。
- `17.0.6` 的维护补丁和发布补丁内容一致，SHA-256 均为 `b1b65b0cf5b9ce48f788b35db03be4b9f9789ce0f72cff130dae5becd6f9b5f8`。
- 两个补丁均已在官方 `v17.0.6` 干净工作树上通过 `git apply --check`，并在维护分支通过反向检查。
- Transcript 固定视口、滚动条、主界面鼠标跟踪及 TUI 重绘定向测试共 192 项通过，`bun run check` 通过。
- Action run [`29836615333`](https://github.com/Frashmeat/message-nav-rail/actions/runs/29836615333) 已完成 native、`omp.exe`、扩展、baseline/modern 内嵌 native 探测和 `custom.2` Release 打包。
- `v17.0.6-custom.2` 已发布；本机当前安装清单仍为 `17.0.6-custom.1`，安装版本必须结合 `installation.json` 和 `omp.exe --version` 确认。
...
```text
message-nav-rail-omp-17.0.6-custom.2-windows-x64.zip
├─ omp.exe
├─ extension/
...
### 17.0.6 发布构建验收（2026-07-21）

GitHub Actions run [`29829441666`](https://github.com/Frashmeat/message-nav-rail/actions/runs/29829441666) 在 Windows x64 runner 上于 23 分 58 秒内完成全部步骤，包括定制补丁应用、扩展验证、native/`omp.exe` 构建、baseline/modern 内嵌 native 探测、Release 打包和 Artifact 上传。下载 Artifact 后完成了以下独立校验：

- 发布 ZIP 与独立 `.sha256` 一致，SHA-256 为 `d5ddc67977494a65d634e439f7fe5227a583ef143d5117d42d58cd173eae1b12`；GitHub Release 资产 digest 与该值一致。
- bundle 包含 `omp.exe`、扩展、安装/卸载脚本、manifest、许可证、说明和内部校验清单。
- `install.ps1 -ValidateOnly` 通过 manifest、文件覆盖率、路径安全和全部内部 SHA-256 校验。
- `manifest.json` 记录 `bundleVersion=17.0.6-custom.1`、`upstreamVersion=17.0.6`、源码提交 `dab8e99213330fa3a2b15dbf66be5294ccf4eeb2` 和 Oh My Pi 基线 `89d6a8f6d14286f32f09ec9c8aa8af7b3451d2d6`。
- 包内 `omp.exe --version` 返回 `omp/17.0.6`。
- [`v17.0.6-custom.1`](https://github.com/Frashmeat/message-nav-rail/releases/tag/v17.0.6-custom.1) 已创建为 prerelease，并上传 ZIP 和独立 `.sha256`。

### 17.0.6-custom.2 安装器修复验收（2026-07-21）

`custom.1` 在未开启 Windows 开发者模式、未使用管理员权限的目标机上调用 `omp plugin install <local-path>` 时，会因 OMP 创建目录符号链接而报 `EPERM`。`custom.2` 改由安装器复制扩展目录并合并 `omp-plugins.lock.json`，不再依赖符号链接权限。

- 项目测试 62 项全部通过，新增用例使用 Windows PowerShell 5.1 完成隔离安装，确认插件是普通目录、其他插件配置和 settings 保持不变。
- 锁文件使用无 BOM UTF-8，已通过 Node 严格 JSON 解析。
- 使用真实 `omp/17.0.6` 在隔离 `USERPROFILE` 和安装目录完成本地预验证。
- GitHub Actions run [`29836615333`](https://github.com/Frashmeat/message-nav-rail/actions/runs/29836615333) 在 13 分 6 秒内完成构建和 Artifact 上传。
- 正式 Artifact 的 `install.ps1 -ValidateOnly` 通过，并在 Windows PowerShell 5.1 下完成隔离完整安装；`omp plugin list`、`omp --version` 和 native `grep` 均通过。
- 正式 ZIP SHA-256 为 `739578cb08a65393f1c61eb590b161f29a88111d6b85d6d24c1dbb932ec5eaba`，与独立 `.sha256` 和 GitHub Release 资产 digest 一致。
- [`v17.0.6-custom.2`](https://github.com/Frashmeat/message-nav-rail/releases/tag/v17.0.6-custom.2) 已创建为 prerelease，并上传 ZIP 和独立 `.sha256`。
...
2026-07-21，run `29836615333` 已完成 `17.0.6-custom.2` 的成功构建、Artifact 校验和 Windows PowerShell 5.1 隔离完整安装。prerelease 由人工确认产物后使用 GitHub CLI 创建，自动创建 Release 的工作流仍未实现。
